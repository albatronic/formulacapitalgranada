...
class EntityBuilder {
    ...
    private function Load() {
        ...
        $this->cabecera = "/**\n";
        $this->cabecera .= " * @copyright GRUPO TREVENQUE\n * @date " . date('d.m.Y H:i:s') . "\n";
        $this->cabecera .= " */\n\n";
        $this->cabecera .= "/**\n";
        $this->cabecera .= " * @orm:Entity({$this->className})\n";
        $this->cabecera .= " */\n";
        ...
        foreach ($this->td->getColumns() as $column) {
            if (!in_array($column['Field'], ColumnasComunes::$columnasExcepcion)) {
                ...
                if ( ($column['Key'] === 'MUL') and (isset($column['ReferencedColumn'])) ) {
                    $relEntity = $column['ReferencedEntity'];
                    $valor = "\t\t\$this->{$column_name} = (\${$column_name} instanceof $relEntity) ? \${$column_name}->getPrimaryKeyValue() : \${$column_name};";
                } elseif (TiposVariables::$tipos[$column['Type']] === 'date') {
                        $valor = "\t\t\$date = new Fecha(\$$column_name);\n";
                        $valor .= "\t\t\$this->$column_name = \$date->getFecha();\n";
                        $valor .= "\t\tunset(\$date);";                    
                } elseif (TiposVariables::$tipos[$column['Type']] === 'string') {  
