...
class TemplateBuilder {
    ...
    public function __construct($conection, $table, $prefijo = '') {
        ...
        $this->filename = $table; //$this->td->getTable();
        $this->sinPrefijo = ucwords(str_replace($prefijo, "", $table));
    ...
    private function helpTemplate() {
        $tmp = "{#\n";
        $tmp .= "  Module: " . $this->sinPrefijo . "\n";
        $tmp .= "  Document : modules\\" . $this->sinPrefijo . "\\help.html.twig\n\n";
        $tmp .= "  copyright: GRUPO TREVENQUE\n";
        $tmp .= "  date " . date('d.m.Y H:i:s') . "\n";
        $tmp .= "#}\n\n";
    ...
    private function indexTemplate() {
        $tmp = "{#\n";
        $tmp .= "  Module: " . $this->sinPrefijo . "\n";
        $tmp .= "  Document : modules\\" . $this->sinPrefijo . "\\index.html.twig\n\n";
        $tmp .= "  copyright: GRUPO TREVENQUE\n";
        $tmp .= "  date " . date('d.m.Y H:i:s') . "\n";
        $tmp .= "#}\n\n";
    ...
    private function filtroTemplate() {
        $tmp = "{# Template filtro.html.twig for " . $this->sinPrefijo . " #}\n";
        $tmp .= "{# -------------------------------------------------------#}\n";
        $tmp .= "<div class='row'>\n";
        $tmp .= "<div class='col-sm-12'>\n";
        $tmp .= "\t<form name='filtro' class='form-inline' id='filtro' action='' method='POST'>\n";
    ...
    private function editTemplate() {
        $tmp = "{# Template edit.html.twig for " . $this->sinPrefijo . " #}\n";
        $tmp .= "{# -------------------------------------------------------#}\n";
        ...
        $tmp .= "{% block mantenimiento %}\n";
        $tmp .= "   {% include '_global/formEdit.html.twig' with {'values': values} %}\n";
        $tmp .= "{% endblock %}";
    ...
    private function formTemplate() {
        $tmp = "{#\n";
        $tmp .= "  Module: " . $this->sinPrefijo . "\n";
        $tmp .= "  Document : modules\\" . $this->sinPrefijo . "\\form.html.twig\n\n";
        $tmp .= "  copyright: GRUPO TREVENQUE\n";
        $tmp .= "  date " . date('d.m.Y H:i:s') . "\n";
        $tmp .= "#}\n\n";
    ...
    private function fieldsTemplate() {
        ...
        $tmp = "{#\n";
        $tmp .= "  Module: " . $this->sinPrefijo . "\n";
        $tmp .= "  Document : modules\\" . $this->sinPrefijo . "\\fields.html.twig\n\n";
        $tmp .= "  copyright: GRUPO TREVENQUE\n";
        $tmp .= "  date " . date('d.m.Y H:i:s') . "\n";
        $tmp .= "#}\n\n";
        ...
        foreach ($this->td->getColumns() as $column) {
            ...
            if (($column['Field'] != $this->td->getPrimaryKey()) and ( !in_array($column['Field'], columnasComunes::$columnasExcepcion))) {
                ...
                if (isset($column['ReferencedSchema'])) {
                    ...
                    switch ($column['Type']) {
                        ...
                        case 'char':
                            $macro = "input";
                            $type = "text";
                            $tagClass = ($column['Length'] >= 30) ? "CampoTextoLargo" : "CampoTextoCorto";
                        ...
                        case 'decimal':
                            $macro = "input";
                            $type = "text";
                            $tagClass = ($column['Length'] >= 8) ? "CampoImporte" : "CampoUnidades";
                            $maxLong = $column['Length'];
                            break;
    ...
    /**
     * Devuelve array con todos los templates
     * @return array
     */
